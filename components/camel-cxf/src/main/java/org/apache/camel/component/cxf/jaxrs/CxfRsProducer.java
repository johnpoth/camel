...
public class CxfRsProducer extends DefaultProducer implements AsyncProcessor {
    ...
    private void invokeAsyncHttpClient(Exchange exchange, final AsyncCallback callback) throws Exception {
        Message inMessage = exchange.getIn();
        ...
        // invoke the client
        client.async().method(httpMethod, entity, new CxfInvocationCallback(exchange, binding, responseClass, callback, genericType));
    }
    ...
    protected void invokeAsyncProxyClient(Exchange exchange, final AsyncCallback callback) throws Exception {
        ...
        final CxfProxyInvocationCallback invocationCallback = new CxfProxyInvocationCallback(target, exchange, binding, callback);
    ...
    private final class CxfInvocationCallback implements InvocationCallback<Response> {
        ...
        private CxfInvocationCallback(Exchange exchange, CxfRsBinding binding, Class<?> responseClass, AsyncCallback callback, Type genericType) {
    ...
    private final class CxfProxyInvocationCallback implements InvocationCallback<Object> {
        ...
        private CxfProxyInvocationCallback(Client client, Exchange exchange, CxfRsBinding binding, AsyncCallback callback) {
            this.exchange = exchange;
            this.binding = binding;
            this.callback = callback;
            this.client = client;
